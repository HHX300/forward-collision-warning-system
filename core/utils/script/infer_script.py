...
def scale_boxes(results, resized_img, img, radio_pad):
    ...
    for result in results:
        if result is not None and len(result):
            result[:, :4] = scale_box(resized_img.shape[2:], result[:, :4], img.shape[:2], ratio_pad=radio_pad).round()
            return result
#endregion
...
def visualize(img, results):
    for result in results:
        if isinstance(result, torch.Tensor):
            ...
            conf = result[4].cpu().numpy()
            cls = result[5].cpu().numpy().astype(int)
            # 画框
            cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
            # 画标签
            cv2.putText(img, f'cls:{cls}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
            cv2.putText(img, f'conf:{conf:.2f}', (int(x2 - (x2 - x1) / 5), y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
        else:
            x1, y1, x2, y2, cls, conf = result[:6].astype(int)
            # 画框
            cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
            # 画标签
            cv2.putText(img, f'cls:{cls}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
